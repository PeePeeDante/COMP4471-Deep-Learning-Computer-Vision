...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        captions_vecs, embed_cache = word_embedding_forward(captions_in, W_embed)
        
        if self.cell_type == 'rnn':
            h, rnn_cache = rnn_forward(captions_vecs, h0, Wx, Wh, b)
            
        elif self.cell_type == 'lstm':
            h, lstm_cache = lstm_forward(captions_vecs, h0, Wx, Wh, b)
        ...
        if  self.cell_type == 'rnn':
            dcaptions_vecs, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, rnn_cache)
        elif self.cell_type == 'lstm':
            dcaptions_vecs, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, lstm_cache)
            
        grads['W_embed'] = word_embedding_backward(dcaptions_vecs, embed_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            idx_max = np.squeeze(np.argmax(scores, axis=2))
            captions[:, i] = idx_max
            prev_h = h
